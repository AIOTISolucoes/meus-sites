...
class WhatsAppWebhookTest(unittest.TestCase):
    ...
    @patch("webhook_manychat.crm.atualizar_status_whatsapp", return_value=True)
    @patch("webhook_manychat.crm.disponivel", return_value=True)
    def test_status_de_entrega_atualiza_crm_mesmo_com_automacao_desligada(
        self, disponivel, atualizar_status,
    ):
        with patch.dict(os.environ, {"WHATSAPP_AUTOMATION_ENABLED": "false"}):
            resposta = self._post_assinado(payload_status_whatsapp())
        self.assertEqual(resposta.status_code, 200)
        self.assertEqual(resposta.get_json()["statuses_updated"], 1)
    ...
    @patch("webhook_manychat.whatsapp_cloud.enviar_imagem", return_value="wamid.imagem")
    @patch("webhook_manychat.tools.cartoes_imoveis")
    @patch("webhook_manychat.whatsapp_cloud.enviar_texto", return_value="wamid.saida")
    @patch("webhook_manychat.agent.responder", return_value="Tenho o cód. 10. Qual prefere?")
    @patch("webhook_manychat.crm.historico_do_lead")
    @patch("webhook_manychat.crm.configuracao_ia")
    @patch("webhook_manychat.crm.registrar_saida")
    @patch("webhook_manychat.crm.registrar_entrada")
    @patch("webhook_manychat.crm.disponivel", return_value=True)
    def test_anexa_link_e_envia_capa_de_imovel_citado(
        self, disponivel, registrar_entrada, registrar_saida, configuracao, historico,
        responder, enviar_texto, cartoes, enviar_imagem,
    ):
        registrar_entrada.return_value = ({"id": "lead-1", "ia_ativa": True}, True)
        configuracao.return_value = {"modo": "automatico", "canais": ["whatsapp"]}
        historico.return_value = [{"role": "user", "content": "Mostre opções"}]
        cartoes.return_value = [{
            "codigo": 10,
            "titulo": "Nature Eusébio",
            "preco": 220000,
            "capa": "https://cdn.test/10.webp",
            "link": "https://imoveis.test/imovel.html?cod=10",
        }]
        with patch.object(
            webhook_manychat,
            "_agentes",
        ...
        self.assertIn("https://imoveis.test/imovel.html?cod=10", texto)
        enviar_imagem.assert_called_once()
        self.assertEqual(enviar_imagem.call_args.args[1], "https://cdn.test/10.webp")
        self.assertEqual(registrar_saida.call_count, 2)
