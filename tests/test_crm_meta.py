...
class CRMMetaTest(unittest.TestCase):
    ...
    @patch("core.crm._rest")
    def test_atualiza_status_whatsapp_pelo_wamid(self, rest):
        rest.side_effect = [
            [{"id": "interacao-1", "metadados": {"wamid": "wamid.saida"}}],
            None,
        ]

        atualizado = crm.atualizar_status_whatsapp(
            "wamid.saida",
            "failed",
            erro_codigo=130497,
        ...
        consulta = rest.call_args_list[0]
        self.assertEqual(consulta.kwargs["params"]["metadados->>wamid"], "eq.wamid.saida")
        alteracao = rest.call_args_list[1]
        self.assertEqual(alteracao.kwargs["json"]["metadados"]["status_envio"], "erro")
        self.assertEqual(alteracao.kwargs["json"]["metadados"]["erro_codigo"], "130497")
