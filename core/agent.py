...
# Alguns modelos podem devolver um rascunho interrompido antes da resposta
# final, por exemplo: ``Nen **...** (continuação)No momento...``. Esse marcador
# e texto interno do raciocinio e nunca deve chegar ao WhatsApp.
_PREFIXO_CONTINUACAO = re.compile(
    r"^\s*.{0,80}?\(\s*continua(?:ção|cao)\s*\)\s*",
...
def _limpar(texto: str) -> str:
    ...
    limpo = _FUNCAO_EM_TEXTO.sub("", texto or "").strip()
    return _PREFIXO_CONTINUACAO.sub("", limpo).strip()
...
def _chamar(client, cadeia, atual, conversa, schemas, config_agente, nome_agente):
    ...
    while atual < len(cadeia):
        ...
        try:
            ...
            # GPT-OSS raciocina antes de responder. Sem `hidden`, o provedor
            # pode devolver rascunhos/continuações junto do texto que iria ao
            # cliente. O raciocínio continua ocorrendo, mas nunca sai no chat.
