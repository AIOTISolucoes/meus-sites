...
def atualizar_status_whatsapp(
    ...
    status: str,
    *,
    erro_codigo: int | str | None = None,
    erro_descricao: str = "",
) -> bool:
    """Atualiza a saida correspondente a um status assinado da Cloud API."""
    identificador = str(wamid or "").strip()
    status_meta = str(status or "").strip().lower()
    if not identificador or status_meta not in {"sent", "delivered", "read", "failed"}:
        return False
    ...
    dados = _rest(
        ...
        params={
            "select": "id,metadados",
            "canal": "eq.whatsapp",
            "metadados->>wamid": f"eq.{identificador}",
            "limit": "1",
        },
        servico=True,
    )
    ...
    metadados["status_meta"] = status_meta
    metadados["status_atualizado_em"] = _agora()
    metadados["status_envio"] = {
        "sent": "enviado",
        "delivered": "entregue",
        "read": "lido",
        "failed": "erro",
    ...
    if status_meta == "failed":
        if erro_codigo is not None:
            metadados["erro_codigo"] = str(erro_codigo)[:30]
        if erro_descricao:
            metadados["erro_descricao"] = str(erro_descricao)[:500]
...
_MARCADOR_NOVA_SESSAO = "[[NOVA_SESSAO_IA]]"


def iniciar_nova_sessao(lead_id: str, *, canal: str = "whatsapp") -> None:
    """Cria um limite de contexto sem apagar o historico do atendimento."""
    _rest(
        "POST",
        "lead_interacoes",
        json={
            ...
            "autor": "sistema",
            "canal": _origem(canal),
            "conteudo": _MARCADOR_NOVA_SESSAO,
            "automatico": True,
        },
        servico=True,
        prefer="return=minimal",
...
def _instante_interacao(valor: Any) -> datetime | None:
    """Converte o timestamp do PostgREST sem deixar dado ruim quebrar o chat."""
    if not valor:
        return None
    try:
...
def historico_do_lead(
    lead_id: str,
    *,
    limite: int = 20,
    reiniciar_apos_horas: float | None = 24,
) -> list[dict[str, str]]:
    """Retorna somente a sessao atual da conversa.

    Um mesmo telefone permanece ligado ao mesmo lead por meses. Sem separar as
    sessoes, preferencias de um atendimento antigo (bairro, quartos, preco)
    voltam como se tivessem sido informadas hoje. Uma pausa de 24 horas abre
    uma nova sessao, alinhada tambem a janela de atendimento do WhatsApp.
    """
    itens = _rest(
        "GET",
        "lead_interacoes",
        params={
            "select": "tipo,direcao,autor,conteudo,criado_em",
            "lead_id": f"eq.{lead_id}",
            "tipo": "in.(mensagem,nota)",
    ...
    itens = list(itens or [])
    if reiniciar_apos_horas is not None and itens:
        sessao = []
        mais_nova = None
        intervalo = timedelta(hours=max(float(reiniciar_apos_horas), 0))
        for item in itens:
            if (
                item.get("tipo") == "nota"
                and item.get("conteudo") == _MARCADOR_NOVA_SESSAO
            ):
                break
            momento = _instante_interacao(item.get("criado_em"))
            if mais_nova is not None and momento is not None and mais_nova - momento > intervalo:
                break
            if item.get("tipo", "mensagem") == "mensagem":
                sessao.append(item)
