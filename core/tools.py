...
def buscar_imoveis(
    ...
    corpo_base = {"p_limite": 5}
    if finalidade in ("venda", "aluguel"):
        corpo_base["p_finalidade"] = finalidade
    if tipo:
        corpo_base["p_tipo"] = tipo
    teto = _para_numero(preco_max)
    if teto > 0:
        corpo_base["p_preco_max"] = teto
    minimo = _para_numero(quartos_min)
    if minimo > 0:
        corpo_base["p_quartos_min"] = int(minimo)

    if isinstance(bairro, list):
        bairros = [str(item).strip() for item in bairro if str(item).strip()]
    elif bairro:
        bairros = [item.strip() for item in str(bairro).split(",") if item.strip()]
    else:
        bairros = []

    # Uma chamada com vários bairros vira uma consulta por bairro porque a RPC
    # recebe somente texto. Sem bairro, mantém a busca ampla original.
    consultas = bairros or [None]
    achados = []
    codigos_vistos = set()

    for bairro_consulta in consultas:
        corpo = dict(corpo_base)
        if bairro_consulta:
            corpo["p_bairro"] = bairro_consulta
        try:
        ...
        for imovel in encontrados:
            identidade = imovel.get("codigo") or imovel.get("id")
            if identidade in codigos_vistos:
                continue
            codigos_vistos.add(identidade)
...
_CODIGO_IMOVEL = re.compile(
    r"\bc[oó]d(?:igo)?\.?\s*#?\s*(\d+)\b",
    re.IGNORECASE,
)


def codigos_de_imoveis(texto: str, *, limite: int = 3) -> list[int]:
    """Extrai códigos citados pela IA, preservando a ordem e sem duplicatas."""
    codigos: list[int] = []
    for valor in _CODIGO_IMOVEL.findall(texto or ""):
        codigo = int(valor)
...
def url_publica_imovel(codigo: int) -> str:
    site = os.environ.get(
        "SITE_PUBLIC_URL", "https://ah-imobiliaria.vercel.app"
    ).strip().rstrip("/")
    return f"{site}/imovel.html?cod={int(codigo)}"


def cartoes_imoveis(codigos: list[int]) -> list[dict]:
    """Busca capas públicas dos imóveis citados, na mesma ordem da resposta."""
    ordem = []
    for valor in codigos[:3]:
        try:
            codigo = int(valor)
        except (TypeError, ValueError):
            continue
        if codigo > 0 and codigo not in ordem:
            ordem.append(codigo)
    if not ordem:
        return []
    ...
    try:
        resposta = requests.get(
            ...
            params={
                "select": "codigo,titulo,tipo,bairro,cidade,preco,capa",
                "codigo": f"in.({','.join(str(c) for c in ordem)})",
            },
            timeout=10,
        )
        resposta.raise_for_status()
    ...
    for codigo in ordem:
        ...
        if not item:
            continue
        resultado.append({**item, "link": url_publica_imovel(codigo)})
    return resultado
...
FERRAMENTAS = {
    ...
    "buscar_imoveis": {
        "funcao": lambda args: buscar_imoveis(
            ...
            tipo=args.get("tipo", ""),
            bairro=args.get("bairro") or args.get("bairros") or "",
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "bairro": {
                            ...
                            "items": {"type": "string"},
                            "description": (
                                "Um bairro/região ou uma lista quando o cliente "
                                "aceitar mais de uma localização"
                            ),
                        },
                        "bairros": {
                            "type": ["array", "null"],
                            "items": {"type": "string"},
                            "description": "Lista alternativa de bairros ou regiões aceitos",
                        },
