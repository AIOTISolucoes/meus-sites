"""Video demo da Savio Barbearia, frame a frame, MOSTRANDO INTERACAO.

Irmao do vendas/gravar-video-elo.py. A coreografia muda porque a
secao-assinatura e outra: aqui o momento e O QUIZ VERDADE OU MITO, que e o que
a propria Savio pergunta nos reels dela. A pessoa chuta, a carta VIRA em 3D e
mostra a resposta; o placar conta os acertos.
...
Ordem: intro -> hero (a frase da parede deles) -> servicos -> O QUIZ (3 cartas,
com um acerto, um erro e outro acerto) -> galeria -> chat.

⚠️ A Savio nao tem secao "onde fica": o endereco com rua e numero nao e
publico, so o bairro. Nao inventar uma secao que o site nao tem.

⚠️ NAO clicar em #btn-quiz: abre o WhatsApp numa aba nova e o video sai do
site.
"""
import json
import pathlib
...
SAIDA = pathlib.Path(sys.argv[1] if len(sys.argv) > 1 else "frames-savio")
URL = "http://localhost:8602/"
CHAT = "http://localhost:8512/?agente=savio_barbearia&embed=true&cor=8a5334"
CHROME = str(pathlib.Path.home() / ".agent-browser/browsers/chrome-150.0.7871.24/chrome.exe")
W, H = 1440, 900
FPS = 25
TETO_ESPERA = 3.0

MSG1 = "Oi! Tem estacionamento ai? E voces ficam em qual bairro?"
MSG2 = "Quanto custa o corte e como funciona o plano de assinatura?"
...
# entao sem isto o video mostraria a carta virando sozinha.
# O anel sai no marrom CLARO do uniforme deles: o quiz fica sobre o vinho
# escuro, e o marrom cheio (#8a5334) sumiria nele.
CURSOR = """
() => {
  ...
  c.innerHTML =
    ...
    'fill="#fff" stroke="rgba(36,16,9,.9)" stroke-width="1.2" stroke-linejoin="round"/></svg>' +
    '<i style="position:absolute;left:-9px;top:-9px;width:44px;height:44px;border-radius:50%;' +
    'border:2px solid rgba(199,141,99,.95);opacity:0;transform:scale(.3)"></i>';
...
with sync_playwright() as pw:
    ...
    # --- 2. hero: a frase da parede do salao. Sem canvas, entao so o hover
    mover(pag, 1120, 320)
    tempo_real(pag, 1.1)
    bx, by = centro_de(pag, '.hero-acoes .btn-vazado')
    mover_ate(pag, bx, by, 0.8, curva=-24)   # so passa por cima: o link pula a pagina
    tempo_real(pag, 0.9)

    # medido na hora: a galeria em trilho e PINADA, entao o percurso dela
    # empurra pra baixo tudo que vem depois
    ys = pag.evaluate("""() => {
        const y = s => document.querySelector(s).getBoundingClientRect().top + window.scrollY;
        const t = document.querySelector('.trilho-interno');
        return {servicos: y('#servicos'), assinatura: y('#assinatura'),
                galeria: y('#galeria'), contato: y('#contato'),
                percurso: t ? t.scrollWidth - window.innerWidth : 0,
    ...
    # --- 4. A SECAO-ASSINATURA: o quiz verdade ou mito
    pos = rolar(pag, pos, ys["assinatura"] + 260, 1.8)
    tempo_real(pag, 1.4)   # da tempo de LER a pergunta antes de chutar

    # 4a. carta 1 — "raspar a cabeca faz o cabelo nascer mais forte": Mito.
    # Acerta de proposito: o placar sai do zero na primeira jogada.
    bx, by = centro_de(pag, '.btn-mito')
    mover_ate(pag, bx, by, 0.7, curva=18)
    clicar(pag, bx, by, 2.6)   # a carta vira em 3D e a explicacao aparece

    # 4b. carta 2 — "aparar as pontas faz crescer mais rapido": tambem Mito.
    # Aqui erra de proposito, chutando Verdade: e a unica forma de mostrar o
    # estado "quase" e a explicacao corrigindo sem gozar da cara de ninguem.
    bx, by = centro_de(pag, "#proxima")
    mover_ate(pag, bx, by, 0.6, curva=-14)
    clicar(pag, bx, by, 1.5)
    bx, by = centro_de(pag, '.btn-verdade')
    mover_ate(pag, bx, by, 0.7, curva=16)
    clicar(pag, bx, by, 2.6)

    # 4c. carta 3 — "bone todo dia causa calvicie": Mito. Acerta, o placar vai
    # a 2, e a resposta e a que manda pro dermatologista: o site nao passa a
    # mao na cabeca de quem tem queda de cabelo.
    bx, by = centro_de(pag, "#proxima")
    mover_ate(pag, bx, by, 0.6, curva=-14)
    clicar(pag, bx, by, 1.5)
    bx, by = centro_de(pag, '.btn-mito')
    mover_ate(pag, bx, by, 0.7, curva=16)
    clicar(pag, bx, by, 2.8)

    # --- 5. galeria horizontal: o percurso e o do proprio trilho, medido acima
    pos = rolar(pag, pos, ys["galeria"] + 40, 1.3)
    pos = rolar(pag, pos, ys["galeria"] + 40 + ys["percurso"], 3.4)
    ...
    # a 1a mostra o pouco que a Savio publica E que o agente SABE: o
    # estacionamento gratis (esta na bio) e o bairro.
    ...
    # a 2a e o contrario: PRECO e REGRA DO PLANO, que a Savio nao publica em
    # lugar nenhum. Tem que responder as duas e recusar as duas sem estimar.
