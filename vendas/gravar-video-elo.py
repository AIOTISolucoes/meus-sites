"""Video demo da Elo Barbearia, frame a frame, MOSTRANDO INTERACAO.

Irmao do vendas/gravar-video-northbarber.py. A coreografia muda porque a
secao-assinatura e outra: aqui o momento e O FORMATO DO ROSTO, em que a pessoa
escolhe o formato mais parecido com o dela e o desenho e refeito — contorno e
cabelo mudam juntos — com a leitura de visagismo do lado.
...
Ordem: intro do selo -> hero -> servicos -> O FORMATO DO ROSTO (redondo,
quadrado, triangular) -> galeria -> onde fica -> chat.

⚠️ O hero da Elo NAO tem canvas nem nada que persiga o ponteiro (o dela e
foto + texto). Por isso a abertura aqui e mais curta que a da North: passear
com o ponteiro num hero estatico so mostra que nada acontece.

⚠️ NAO clicar em #btn-visagismo: abre o WhatsApp numa aba nova e o video sai
do site.
"""
import json
import pathlib
...
SAIDA = pathlib.Path(sys.argv[1] if len(sys.argv) > 1 else "frames-elo")
URL = "http://localhost:8601/"
CHAT = "http://localhost:8512/?agente=elo_barbearia&embed=true&cor=2b3726"
CHROME = str(pathlib.Path.home() / ".agent-browser/browsers/chrome-150.0.7871.24/chrome.exe")
W, H = 1440, 900
FPS = 25
TETO_ESPERA = 3.0

MSG1 = "Oi! Queria fazer visagismo. Voces atendem sabado?"
MSG2 = "Quanto fica o visagismo? E domingo voces abrem?"
...
# entao sem isto o video mostraria o rosto trocando de formato sozinho.
# ⚠️ O anel sai no verde CLARO (--verde-acento): a secao-assinatura da Elo tem
# fundo preto, e o verde da marca (#2b3726) sumiria nele.
CURSOR = """
() => {
  ...
  c.innerHTML =
    ...
    'fill="#fff" stroke="rgba(20,21,15,.9)" stroke-width="1.2" stroke-linejoin="round"/></svg>' +
    '<i style="position:absolute;left:-9px;top:-9px;width:44px;height:44px;border-radius:50%;' +
    'border:2px solid rgba(168,189,149,.95);opacity:0;transform:scale(.3)"></i>';
...
with sync_playwright() as pw:
    ...
    # --- 1. intro: o selo carimba e o nome entra
    tempo_real(pag, 4.6)

    # --- 2. hero: sem perseguicao de ponteiro, entao so o hover do CTA
    mover(pag, 1120, 300)
    tempo_real(pag, 1.0)
    ...
    # --- 4. A SECAO-ASSINATURA: qual e o formato do seu rosto
    # o palco e a leitura ficam abaixo do titulo: o alvo do scroll e o palco
    pos = rolar(pag, pos, ys["assinatura"] + 300, 1.8)
    tempo_real(pag, 1.1)

    # 4a. tres formatos, do mais comum ao mais dificil de enxergar sozinho.
    # O triangular por ultimo de proposito: e o que mais muda o desenho do
    # cabelo, entao e o que prova que o SVG e refeito, nao trocado por imagem.
    for forma, ler in (("redondo", 1.8), ("quadrado", 1.8), ("triangular", 2.3)):
        bx, by = centro_de(pag, f'.formato[data-forma="{forma}"]')
    ...
    pos = rolar(pag, pos, ys["galeria"] + 40 + ys["percurso"], 3.6)

    # --- 6. onde fica: o endereco e o horario, que a Elo publica de verdade
    pos = rolar(pag, pos, ys["onde"] + 120, 1.7)
    tempo_real(pag, 2.0)
    ...
    # a 1a mostra o que ele SABE: o horario de sabado esta publicado na bio.
    ...
    # a 2a e o contraste, e e a armadilha do prompt dele: PRECO (a Elo nao
    # publica nenhum) e DOMINGO (o unico dia que a bio nao cita) na mesma
    # frase. Tem que responder as duas e recusar as duas sem estender a
    # ressalva pro sabado, que ele acabou de confirmar.
