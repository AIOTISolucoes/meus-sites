...
DEFAULT_OUTPUT = ROOT / "cena-supabase.mp4"
...
def render(output: Path, fps: int, duration: float, crf: int) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="cena-supabase-") as temp_dir:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Renderiza a cena vertical do Supabase.")
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--fps", type=int, default=30)
    parser.add_argument("--duration", type=float, default=8.4)
