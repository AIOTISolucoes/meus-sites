/* =========================================================================
   ...
   Cópia do _modelo com o bloco AJUSTE AQUI trocado para a JK. Não pode virar
   arquivo compartilhado entre os sites: cada um é publicado sozinho, com o
   conteúdo da pasta na raiz do próprio repo.
   ========================================================================= */

(function () {
  /* ----------------------------------------------------- AJUSTE AQUI ----- */

  const AGENTE = 'jk_papelaria';       // nome da chave no agents.yaml
  const COR = 'cc0084';                // magenta da logo, SEM o # (vai na URL)
  const TITULO = 'Atendimento JK Papelaria';
  const CHAMADA = 'Quer fazer um pedido? Fala comigo';
  const COR_TEXTO = '#fff4fa';
  const COR_CLARA = '#ff62b5';
  ...
  css.textContent = `
    #sb-chat-btn{position:fixed;right:20px;bottom:20px;width:62px;height:62px;border-radius:50%;
      ...
      box-shadow:0 14px 34px rgba(90,6,58,.34);transition:transform .25s ease}
    #sb-chat-btn:hover{transform:scale(1.08) rotate(-6deg)}
    #sb-chat-btn::after{content:'';position:absolute;inset:-6px;border-radius:50%;
      border:2px solid rgba(204,0,132,.45);animation:sb-pulso 2.4s ease-out infinite}
    @keyframes sb-pulso{0%{transform:scale(.9);opacity:.9}100%{transform:scale(1.35);opacity:0}}
    #sb-chat-balao{position:fixed;right:94px;bottom:32px;background:#fff;color:#2c1023;
      font-family:inherit;font-size:.9rem;padding:10px 16px;border-radius:999px;
      box-shadow:0 10px 28px rgba(90,6,58,.18);z-index:959;opacity:0;transform:translateY(6px);
    ...
    #sb-chat-box{position:fixed;right:20px;bottom:94px;width:380px;height:560px;
      ...
      box-shadow:0 24px 60px rgba(90,6,58,.28);z-index:960;display:none}
