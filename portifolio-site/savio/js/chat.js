...
(function () {
  ...
  const AGENTE = 'savio_barbearia';      // nome da chave no agents.yaml
  const COR = '8a5334';                  // o marrom do uniforme deles, SEM o #
  const TITULO = 'Atendimento da Savio Barbearia';
  const CHAMADA = 'Bora marcar? Fala comigo';   // balãozinho que aparece sozinho
  const COR_TEXTO = '#ffffff';           // texto sobre a cor da marca
  const COR_CLARA = '#5c2f16';           // 2º tom do gradiente do botão
  ...
  css.textContent = `
    #sb-chat-btn{position:fixed;right:20px;bottom:20px;width:62px;height:62px;border-radius:50%;
      ...
      box-shadow:0 14px 34px rgba(60,25,10,.42);transition:transform .25s ease}
    #sb-chat-btn:hover{transform:scale(1.08) rotate(-6deg)}
    #sb-chat-btn::after{content:'';position:absolute;inset:-6px;border-radius:50%;
      border:2px solid rgba(138,83,52,.55);animation:sb-pulso 2.4s ease-out infinite}
    @keyframes sb-pulso{0%{transform:scale(.9);opacity:.9}100%{transform:scale(1.35);opacity:0}}
    #sb-chat-balao{position:fixed;right:94px;bottom:32px;background:#fff;color:#241009;
      font-family:inherit;font-size:.9rem;padding:10px 16px;border-radius:999px;
      box-shadow:0 10px 28px rgba(60,25,10,.22);z-index:959;opacity:0;transform:translateY(6px);
    ...
    #sb-chat-box{position:fixed;right:20px;bottom:94px;width:380px;height:560px;
      ...
      box-shadow:0 24px 60px rgba(60,25,10,.34);z-index:960;display:none}
