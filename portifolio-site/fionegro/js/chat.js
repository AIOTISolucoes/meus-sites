...
(function () {
  ...
  const AGENTE = 'atendimento';        // nome da chave no agents.yaml
  const COR = '1f4e96';                // cor da marca, SEM o # (vai na URL)
  const TITULO = 'Atendimento';        // title do iframe (acessibilidade)
  const CHAMADA = 'Quer agendar? Fala comigo';   // balãozinho que aparece sozinho
  const COR_TEXTO = '#eef5ff';         // texto sobre a cor da marca
  const COR_CLARA = '#4d86d6';         // 2º tom do gradiente do botão
  ...
  css.textContent = `
    ...
    #sb-chat-btn::after{content:'';position:absolute;inset:-6px;border-radius:50%;
      border:2px solid rgba(31,78,150,.45);animation:sb-pulso 2.4s ease-out infinite}
