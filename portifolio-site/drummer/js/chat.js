...
(function () {
  ...
  const AGENTE = 'drummer_barbearia';  // nome da chave no agents.yaml
  const COR = 'a97b2e';                // o latão das luminárias, SEM o #
  const TITULO = 'Atendimento Drummer Barbearia';   // title do iframe
  const CHAMADA = 'Quanto fica o seu combo?';       // balãozinho sozinho
  const COR_TEXTO = '#f7f2e6';         // texto sobre a cor da marca
  const COR_CLARA = '#d9a44e';         // 2º tom do gradiente do botão
  ...
  css.textContent = `
    ...
    #sb-chat-btn::after{content:'';position:absolute;inset:-6px;border-radius:50%;
      border:2px solid rgba(169,123,46,.6);animation:sb-pulso 2.4s ease-out infinite}
