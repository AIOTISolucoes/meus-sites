...
(function () {
  ...
  const AGENTE = 'diretoria_barber';   // nome da chave no agents.yaml
  const COR = 'e8a71d';                // o âmbar da logo, SEM o #
  const TITULO = 'Atendimento Diretoria Barber';    // title do iframe
  const CHAMADA = 'Abrimos domingo de manhã!';      // balãozinho sozinho
  const COR_TEXTO = '#16110c';         // PRETO: branco sobre o âmbar some
  const COR_CLARA = '#f6c445';         // 2º tom do gradiente do botão
  ...
  css.textContent = `
    ...
    #sb-chat-btn::after{content:'';position:absolute;inset:-6px;border-radius:50%;
      border:2px solid rgba(232,167,29,.6);animation:sb-pulso 2.4s ease-out infinite}
