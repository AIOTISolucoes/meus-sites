/* Assistente virtual (agente "speculari_otica" do agents.yaml, servido pelo app Streamlit).
   IDs #ai-btn, #ai-box, #ai-close são usados pelos testes e pelo roteiro de vídeo. */
(() => {
  const config = {
    agent: 'speculari_otica',
    color: 'd9b877',
    title: 'Assistente Speculari',
    label: 'Conversar',
    fallbackHref: 'https://www.instagram.com/speculariotica/',
    fallbackText: 'Prefere o Instagram? @speculariotica'
  };
  ...
  style.textContent = `
  #ai-btn{position:fixed;right:max(16px,env(safe-area-inset-right));bottom:max(16px,env(safe-area-inset-bottom));z-index:60;display:inline-flex;align-items:center;gap:.5rem;min-height:52px;padding:0 1.15rem 0 .95rem;border:0;border-radius:2px;background:oklch(0.935 0.02 85);color:oklch(0.2 0.006 260);font:700 1rem/1 "Urbanist",system-ui,sans-serif;box-shadow:0 14px 34px -10px oklch(0 0 0 / .6);cursor:pointer;transition:transform .45s cubic-bezier(.22,1,.36,1)}
  #ai-btn:hover{transform:translateY(-2px)}
  #ai-btn i{font-size:1.45rem;color:oklch(0.2 0.006 260)}
  #ai-btn:focus-visible,#ai-close:focus-visible,#ai-fallback:focus-visible{outline:3px solid #${config.color};outline-offset:3px}
  #ai-box{position:fixed;right:max(16px,env(safe-area-inset-right));bottom:calc(max(16px,env(safe-area-inset-bottom)) + 64px);z-index:60;width:min(390px,calc(100vw - 32px));height:min(580px,calc(100dvh - 120px));display:none;grid-template-rows:52px 1fr auto;background:#fff;border-radius:4px;overflow:hidden;box-shadow:0 30px 70px -20px oklch(0 0 0 / .65);border:1px solid oklch(0.28 0.006 260)}
  ...
  #ai-head{display:flex;align-items:center;justify-content:space-between;padding-left:16px;background:oklch(0.165 0.004 260);color:oklch(0.935 0.02 85);font:700 .98rem/1.2 "Urbanist",system-ui,sans-serif}
  ...
  #ai-status{display:grid;place-items:center;padding:24px;text-align:center;color:oklch(0.4 0.006 260);font:400 .95rem/1.5 "Urbanist",system-ui,sans-serif}
  #ai-fallback{display:block;padding:12px;text-align:center;background:oklch(0.915 0.003 260);color:oklch(0.2 0.006 260);font:700 .9rem/1.2 "Urbanist",system-ui,sans-serif;text-decoration:none}
  ...
  btn.innerHTML = `<i class="ph-light ph-chat-circle-text" aria-hidden="true"></i><span>${config.label}</span>`;
  ...
  const load = () => {
    ...
    const timer = setTimeout(() => {
      if (frame.hidden) status().textContent = 'O assistente está demorando para abrir. Você pode falar com a ótica pelo Instagram, no link abaixo.';
    }, 15000);
  ...
  const setOpen = open => {
    ...
    btn.innerHTML = open
      ? '<i class="ph-light ph-x" aria-hidden="true"></i><span>Fechar</span>'
      : `<i class="ph-light ph-chat-circle-text" aria-hidden="true"></i><span>${config.label}</span>`;
