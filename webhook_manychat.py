...
from core import agent, config, crm, leads, meta_leads, tools, whatsapp_cloud
...
def _processar_mensagem_whatsapp(mensagem: dict[str, str]) -> str:
    ...
    codigos = tools.codigos_de_imoveis(resposta_cliente)
    cartoes = tools.cartoes_imoveis(codigos)
    if cartoes:
        links_faltantes = [
            f"Cód. {item['codigo']}: {item['link']}"
            for item in cartoes
            if item["link"] not in resposta_cliente
        ]
        if links_faltantes:
            resposta_cliente = (
                resposta_cliente.rstrip()
                + "\n\nVer detalhes e todas as fotos:\n"
                + "\n".join(links_faltantes)
            )[:4096]
    ...
    # O texto e os links são a entrega principal. As capas vêm depois como
    # cartões visuais; se uma imagem falhar, o cliente ainda consegue abrir o
    # imóvel e não recebe uma resposta textual duplicada no retry do webhook.
    for item in cartoes:
        capa = str(item.get("capa") or "").strip()
        if not capa:
            continue
        preco = f"R$ {float(item.get('preco') or 0):,.0f}".replace(",", ".")
        legenda = (
            f"Cód. {item['codigo']} — {item.get('titulo') or 'Imóvel'}\n"
            f"{preco}\nVeja os detalhes e todas as fotos:\n{item['link']}"
        )
        try:
            wamid_imagem = whatsapp_cloud.enviar_imagem(remetente, capa, legenda)
            crm.registrar_saida(
                lead_crm["id"],
                legenda,
                canal="whatsapp",
                external_id=f"{resposta_id}:image:{item['codigo']}",
                automatico=True,
                metadados={
            ...
            try:
                crm.registrar_erro(
                    lead_crm["id"],
                    f"Falha ao enviar a capa do imóvel {item['codigo']}.",
                    canal="whatsapp",
                )
            except crm.CRMErro:
                print("Aviso: capa não enviada e falha não registrada no CRM.")
        except crm.CRMErro:
            # A imagem já foi aceita pelo WhatsApp; não força retry que poderia
            # duplicá-la só porque o registro de auditoria falhou.
            print("Aviso: capa enviada, mas não registrada no CRM.")
...
def receber_webhook_whatsapp():
    ...
    statuses = whatsapp_cloud.extrair_statuses(dados)
    statuses_atualizados = 0
    if statuses:
        if not crm.disponivel():
            return jsonify({"received": False, "error": "crm_unavailable"}), 503
        try:
            for status in statuses:
                if crm.atualizar_status_whatsapp(
                    status["id"],
                    status["status"],
                    erro_codigo=status.get("erro_codigo"),
                    erro_descricao=status.get("erro_descricao", ""),
                ):
                    statuses_atualizados += 1
        except crm.CRMErro:
            return jsonify({"received": False, "error": "status_processing_failed"}), 500
        ...
        return jsonify(
            {
                "received": True,
                "processed": 0,
                "statuses_updated": statuses_atualizados,
            }
        )
